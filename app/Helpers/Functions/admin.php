 ...
 * @return string
 */
function ajaxCheckboxDisplayDate($id, $table, $field, $fieldValue = null)
{
    $lineId = $field.$id;
    $lineId = str_replace('.', '', $lineId); // fix JS bug (in admin layout)
    $data = 'data-table="' . $table . '" 
    ...
    if (isset($fieldValue) && $fieldValue) {  
        $urlPath = 'post/'.$id.'/unapprove';
        $actionUrl = admin_url($urlPath);
        $html = '<i id="' . $lineId . '" class="admin-single-icon fa fa-toggle-on" aria-hidden="true"></i>';
    } else {
        $urlPath = 'post/'.$id.'/approve';
        $actionUrl = admin_url($urlPath);
        $html = '<i id="' . $lineId . '" class="admin-single-icon fa fa-toggle-off" aria-hidden="true"></i>';
    }
    $html = '<a href="' . $actionUrl . '" class="" ' . $data . '>' . $html . '</a>';
