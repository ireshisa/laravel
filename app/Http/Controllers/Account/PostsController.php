...
//        dd($pagePath);
    ...
    public function getPage($pagePath) {
        ...
        switch ($pagePath) {
            ...
            case 'company-followers':
                ...
                return $this->getCompanyFollowers();
            case 'reviews':
    ...
    public function getApplicants() {
        ...
        MetaTag::set('description', t('Job Applicants :app_name', ['app_name' => config('settings.app.app_name')]));
    ...
    public function getInterviews() {
        ...
        // Meta Tags
        MetaTag::set('title', t('Interviews'));
        MetaTag::set('description', t('Interviews :app_name', ['app_name' => config('settings.app.app_name')]));

        return view('account.interviews', $data);
    }
    ...
    public function getReviews() {
        ...
        $data['reviews'] = $reviews->paginate($this->perPage);
        $data['isCompany'] = $isCompany;
        $data['company'] = $company;

        // Meta Tags
        MetaTag::set('title', t('Reviews'));
        MetaTag::set('description', t('Reviews :app_name', ['app_name' => config('settings.app.app_name')]));
...
//        if (auth()->user()->user_type_id == 1)
//        {
            $followingCompanies = auth()->user()->followingCompanies;
            $data['companies'] = auth()->user()->followingCompanies()->with('company','follower')->paginate($this->perPage);
            // $data['companies'] =  $data['companies']->load('company','follower');
            //  $data['companies'] = $user->load()->get();
            $data['companyCount'] = $followingCompanies->count();
//            dd($data);
//        }
//        else {
//            //$followingCompanies = auth()->user()->followingCompanies;
...
//dd($data);
        // Meta Tags
        MetaTag::set('title', t('Followers'));
        MetaTag::set('description', t('My followers :app_name', ['app_name' => config('settings.app.app_name')]));

        return view('account.followers', $data);
    }
    ...
    public function getCompanyFollowers() {
        ...
        $followingCompanies = auth()->user()->myCompaniesFollowers;
        $data['followers'] = auth()->user()->myCompaniesFollowers()->paginate($this->perPage);
        $data['followersCount'] = $followingCompanies->count();

        // Meta Tags
        MetaTag::set('title', t('Followers'));
        MetaTag::set('description', t('My followers :app_name', ['app_name' => config('settings.app.app_name')]));

        return view('account.company-followers', $data);
    }
    ...
    public function getMyPosts($postId = null) {
        ...
        $data = [];
        $data['posts'] = $this->myPosts->withCount(['messages'=>function ($q) {
