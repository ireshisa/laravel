   ...
   public $perPage = 10;
    ...
    public function index(Request $request)
    {
       ...
       // dd($sectors);
        ...
        if (auth()->check())
        {
            $messages = Message::where('is_approved',1)->where('from_user_id',auth()->user()->id)->orWhere('to_user_id',auth()->user()->id)->get();
            foreach($messages as $message)
            {
                array_push($cands_array,$message->from_user_id);
                array_push($cands_array,$message->to_user_id);
            }
        }
      $cands_array =  array_unique($cands_array);
        ...
        return view('customs.talents.index')->with('users', $users)->with('saved',$saved_candidates)->with('connected',$cands_array)->with('count', $count)->with('categories',$sectors)->with('provinces', $provinces)->with('post_types', $postTypes)->with('result', $result)->with('reviews', $reviews);        
        ...
        foreach ($this->filters as $filter => $options) {
            ...
            if ($value = $request->{$filter}) {
               ...
               // $value = $options['condition'] == 'LIKE' ? $value . '%' : $value;
                ...
                {
                    $a = ["6 Months","1 Year","2 Years","3 Years","4 Years","5 Years","more than 5 Years","None"];
                    $arr = array();
                    $splitted = explode(',',$value);
                    foreach ($splitted as $sp)
                    {
                    ...
                    $builder->whereIn($options['field'],$arr);
                ...
                if ($filter == "gender")
                {
                    ...
                    $builder->whereIn($options['field'],$splitted);
...
//dd($user);
        ...
        $reviews = Review::with('report.status','reviewer.myCompanies')->where("user_id", $id)->where("reviews.status", 0)->orderBy('id', 'desc')->get();
       //   dd($reviews);
        ...
        if (auth()->check())
        {
            // $connection_id = Message::whereIn('from_user_id', [auth()->user()->id])->where('post_id',$post_id)->get()->first();
        // $connection_connectId = Message::whereIn('from_user_id', [$id, auth()->user()->id])->WhereIn('to_user_id', [$id, auth()->user()->id])->where('post_id',$post_id)->whereIn('delete_request', ['0'])->first();
         $connected = Message::where('to_user_id', auth()->user()->id)->orWhere('from_user_id',auth()->user()->id)->get();
         $connected = $connected->filter(function ($value,$key) use($id) {
             if ($value->to_user_id == $id || $value->from_user_id == $id)
             {
                 return $value;
             }
         });
         $connected = $connected->where('deleted_by',NULL)->where('is_approved',1);
        ...
        if ($connected->count() > 0)
         {
             ...
             if (auth()->user()->user_type_id == 1)
             {
                ...
                $company = $company_user->myCompanies[0]->name;
                 $refereeData = new \stdClass();
             
             $refereeData->employer = $company;
         ...
         if ( (auth()->check() && auth()->user()->id==1) || (auth()->check() && auth()->user()->id==166))
         {
                     $show_details = true;
         }
        ...
        return view('customs.talents.show', ['show_details'=>$show_details,'refereeData'=>$refereeData,'saved'=>$saved_candidates,'reviewData'=>null,'userReview'=>(!empty($userReview->userReview)?$userReview->userReview:null),'reviews' => $showReviews,'avg_rating'=>$averageRating,'jobs'=>$jobs, 'user' => $user, 'endorsementCount' => $count, 'connection_id' => $connection_id, 'post_id' => $post_ids, 'connection_connectId' => $connection_connectId]);
    }
...
//        }
		...
		// Save
            ...
            $email->from_user = $user->name;
            if (count($user->myCompanies)>0)
            {
            ...
            $email->type=2;
            $email->status = 1;
            $email->name2 = $email->from_user;
            $user_to->notify(new ConnectStatus($email));
}
