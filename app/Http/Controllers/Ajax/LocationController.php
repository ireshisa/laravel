...
class LocationController extends FrontController
{
	...
	public function getAdmin1Cities($subAdmin1Code)
    {
        $cities = City::where('subadmin1_code',$subAdmin1Code)->orderBy('name','asc')->pluck('id','name');
        $cities_arr = array();
        foreach ($cities as $key=>$value)
        {
            array_push($cities_arr,["id"=>$value,"text"=>$key]);
        }
//       dd($cities_arr);
