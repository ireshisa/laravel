...
class DashboardController extends PanelController
{
	...
	public function __construct()
	{
		...
		// Count Ads
		$countActivatedPosts = Post::approved()->count();
		$countUnactivatedPosts = Post::unapproved()->count();
	...
	private function getLatestPostsChart($statDayNumber = 30)
	{
		...
		for ($i = 1; $i <= $statDayNumber; $i++) {
			...
			// Ads Stats
			$countActivatedPosts = Post::approved()
				->where('created_at', '>=', $date)
				->where('created_at', '<=', $date . ' 23:59:59')
				->count();
			
			$countUnactivatedPosts = Post::unapproved()
				->where('created_at', '>=', $date)
				->where('created_at', '<=', $date . ' 23:59:59')
				->count();
