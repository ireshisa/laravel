...
//				->where('tmp_token', $postIdOrToken)
		...
		view()->share('admin_divs', $admin_divs);
		// Meta Tags
	 ...
	 */
	public function postUpdateForm($postIdOrToken=null,PostRequest $request)
	{
		// Get Post
       // dd(getSegment(2));
		if (getSegment(2) == 'create') {
...
//				->where('tmp_token', $postIdOrToken)
		...
		$post->ip_addr = Ip::get();
        $post->is_filled = ($request->has('position_filled')?$request->position_filled:0);
