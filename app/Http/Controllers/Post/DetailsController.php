...
<p>
    ...
    <br>'.auth()->user()->name.
		...
		if(auth()->check())
        {
            $applied = Message::where('from_user_id',auth()->user()->id)->orWhere('to_user_id',auth()->user()->id)->get();
            $applied = $applied->where('post_id',$post->id);
		...
		view()->share('showConnect',((!empty($applied) && count($applied) > 0)?false:true));
		/*
		// Expiration Info
...
//		}
	   ...
	   //  dd($test);
	    ...
	    $employer = User::with('myCompanies')->findOrFail($message->to_user_id);
	    $user_from = User::findOrFail(auth()->user()->id);
	    
	    if (count($employer->myCompanies) > 0)
	    {
$user_to_name = $employer->myCompanies[0]->name;
	    }
		$notification = "New Connection request Received From ";
// 		if (\auth()->user()->user_type_id == 1)
//         {
//             $user = User::with('myCompanies')->findOrFail(\auth()->user()->id);
//             $notification.=$user->myCompanies[0]->name." (Employer)";
//         }
// 		else {
            $notification.=$user->first_name." (Employee)";
        // }
        $message->save();
        $conn = Notification::create(["notification"=>$notification,"to_user_id"=>$post->user_id,"type"=>"success","created_by"=>\auth()->user()->id,"url"=>url('/account/pending')]);
...
//		// Save
		...
		flash($msg)->success();
        $email = new \stdClass();
        $email->name=$user_to_name;
        $email->title=$post->title;
        $email->from_user=$user->firstname;
        $email->type=1;
        $email->status=1;
        $email->name2=$user->firstname;
        $employer->notify(new ConnectStatus($email));
