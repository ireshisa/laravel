...
class SocialController extends FrontController
{
	...
	{
	    if (isset($_GET['type'])) Session::put('social_user_type', $_GET['type']);
			...
			if (empty($user))
			{
				...
				if (empty($user)) {
					$userInfo = [
						...
						'ip_addr'        => Ip::get(),
                        'user_type_id'   => (Session::has('social_user_type')) ? (int) Session::get('social_user_type') : null,
						'verified_email' => 1,
						'verified_phone' => 1,
						'provider'       => $provider,
					...
					// remove social user type
                    if (Session::has('social_user_type')) Session::remove('social_user_type');
