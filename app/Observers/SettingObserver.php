		...
		if (array_key_exists('distance_calculation_formula', $setting->value)) {
			try {
				...
				foreach ($customFunctions as $function) {
					...
					Cache::forget('checkIfMySQLFunctionExists.' . $function);
				}
			} catch (\Exception $e) {}
		
			// Create the MySQL Distance Calculation function, If doesn't exist.
			if (!DBTool::checkIfMySQLDistanceCalculationFunctionExists($setting->value['distance_calculation_formula'])) {
				$res = DBTool::createMySQLDistanceCalculationFunction($setting->value['distance_calculation_formula']);
			}
		}
