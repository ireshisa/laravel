...
class AppServiceProvider extends ServiceProvider
{
	...
	public function boot()
	{
		...
		if(auth()->check())
        {
            $unread = Message::where('to_user_id',auth()->user()->id)->where('is_read',0)->count();
		...
		if (!DBTool::checkIfMySQLDistanceCalculationFunctionExists(config('settings.listing.distance_calculation_formula'))) {
			$res = DBTool::createMySQLDistanceCalculationFunction(config('settings.listing.distance_calculation_formula'));
		}
