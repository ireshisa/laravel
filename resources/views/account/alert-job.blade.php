		...
		<div class="container">
			<div class="row">
				...
				<div class="col-md-9 page-content">
					<div class="inner-box">
						@if ($pagePath=='job-alerts')
							<h2 class="title-2"><i class="icon-docs"></i> {{ ('Jobs List') }} </h2>
						@endif
         ...
         <!--                               </button>-->
								...
								<table id="addManageTable" class="table table-striped table-bordered add-manage-table table demo"
									...
									<thead>
    									<tr>
    										...
    										<th> {{ t('Photo') }}</th>
											<th data-sort-ignore="true">{{__('Tags')}}</th>
    										<th data-sort-ignore="true"> {{ t('Ads Details') }} </th>
    										<th data-type="numeric"> --</th>
    										<th data-sort-ignore="true">{{__('Action')}}</th>
    									</tr>
									</thead>
									<tbody>

									<?php
                                    if (isset($jobalertposts) && $jobalertposts->count() > 0):
									foreach($jobalertposts as $key => $post):
										// Fixed 1
									...
									<tr>
										...
										<td>
										    <a href="{{ $postUrl }}" class="btn btn-default btn-sm"><i class="icon-eye"></i></a>
									    </td>
                        ...
                        <nav>
                            {{ (isset($jobalertposts)) ? $jobalertposts->links() : '' }}
                        </div>
