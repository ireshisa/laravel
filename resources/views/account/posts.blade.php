				...
				<div class="col-md-9 page-content">
					<div class="inner-box">
						@if ($pagePath=='my-posts')
							<h2 class="title-2"><i class="icon-docs"></i> {{ t('My Ads') }} 
							<a href="{{url('/posts/create')}}" class="btn btn-outline-primary btn-md float-right mx-3 " style="margin-top: -6px; width: 140px;"><i class="icon-plus"></i> {{ ('Create') }}</a>
							</h2>
						...
						@endif
                        <p class="mb-4">Employer view and manage job posts here.</p>
						<div class="table-responsive">
							<form name="listForm" method="POST" action="{{ lurl('account/'.$pagePath.'/delete') }}">
								...
								<table id="addManageTable" class="table table-striped table-bordered add-manage-table table demo"
									...
									<tr class="{{(($post->is_filled == 1)?'opac-50':'')}}">
										...
										<td style="width:58%" class="items-details-td">
											<div>
												...
												<p>
													...
													@endif
													<strong><i class="fa fa-link"></i> <span class="badge badge-success">{{$post->messages_count}}</span> </strong>
												</p>
											</div>
										</td>
										...
										<td style="width:10%" class="action-td">
											<div>
												@if (in_array($pagePath, ['my-posts']) and $post->user_id==$user->id and $post->archived==0)
													<p class="text-center">
														<a class="btn btn-view btn-sm" style="color: #ffff !important; background: #00be6c; width: 4rem;" href="{{ $post->id }}">
												...
												@if (in_array($pagePath, ['my-posts']) and $post->user_id==$user->id and $post->archived==0)
													<p class="text-center">
                                                        <a class="btn btn-primary btn-sm" style="width: 4rem;" href="{{ \App\Helpers\UrlGen::editPost($post) }}">
...
{{--												@endif--}}
												<p class="text-center">
                                                    <a class="btn btn-danger btn-sm delete-action" style="width: 4rem;" href="{{ lurl('account/'.$pagePath.'/'.$post->id.'/delete') }}">
                                                        <i class="fa fa-trash"></i> {{ t('Delete') }}
                                                    </a>
                                                </p>
											</div>
