                                ...
                                <tbody>
                                    ...
                                    if (isset($followers) && $followers->count() > 0):
                                        foreach ($followers as $key => $follower):
                                             if (!empty($follower->follower->name )) :
                                            ?>
                                            <tr>
                                                <td class="add-img-selector">
                                                    <div class="checkbox">
                                                        <label><input type="checkbox" name="entries[]" value="{{ $follower->id }}"></label>
                                                    </div>
                                                </td>
                                                <td>
                                                    <div style="word-break:break-all;">                                                        
                                                        <strong>Name:</strong> {{ $follower->follower->name }}<br>
{{--                                                        <strong>{{ t('Email') }}:</strong>&nbsp;{{ $follower->follower->email }}:<br>--}}
{{--                                                        <strong>{{ t('Phone') }}:</strong>&nbsp;{{ $follower->follower->phone }}:<br>--}}
                                                        <strong>Following Company:</strong>&nbsp;{{ $follower->company->name }}<br>
                                                    </div>
                                                </td>
                                                <td class="action-td">
                                                    <div>
                                                        <p>
                                                            <a class="btn btn-default btn-sm" href="{{ lurl('search-talent/seeker/' . $follower->follower->id) }}" style="width: 4rem;">
                                                                <i class="icon-eye"></i> {{ t('View') }}
                                                            </a>
                                                        </p>
<!--                                                        <p>
                                                            <a class="btn btn-danger btn-sm delete-action" href="{{ lurl('account/conversations/' . $follower->id . '/delete') }}">
                                                                <i class="fa fa-trash"></i> {{ t('Delete') }}
                                                            </a>
                                                        </p>-->
                                                    </div>
                    ...
                    <nav class="" aria-label="">
                        {{ (isset($followers)) ? $followers->links() : '' }}
                    </nav>
