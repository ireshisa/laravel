		...
		<div class="modal-content">
			...
			<div class="modal-header">
				<h4 class="modal-title">
					<i class="icon-mail-2"></i> {{ ('Connect Employer') }}
				</h4>
			...
			<form role="form" method="POST" action="{{ lurl('posts/' . $post->id . '/contact') }}" enctype="multipart/form-data">
				...
				<div class="modal-body">
					...
					@include('account.resume._form', ['originForm' => 'message'])
						<?php $coverLetterError = (isset($errors) and $errors->has('cover_letter')) ? ' is-invalid' : ''; ?>
						<div class="form-group required">
							<label class="col-form-label {{$coverLetterError}}" for="cover_letter">Cover Letter :</label>
							<textarea class="form-control "
									  id="cover-letter"
									  name="cover_letter"
									  rows="10"
							>{{(replaceCoverLetter($coverLetter,$cLReplacements))}}</textarea>
						</div>
				...
				<div class="modal-footer">
					...
					<button type="submit" class="btn btn-success pull-right">{{ ('Connect') }}</button>
				</div>
			</form>
		</div>
	</div>
